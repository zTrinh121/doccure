...
import { appointmentPrefix } from 'src/utils/apiConstants';
...
export const getAppointments = async ({
  ...
  return authAxiosInstance.get(
    `${appointmentPrefix}/all${
      startDate && endDate ? '/date' : ''
    }?offset=${offset}&limit=${limit}${status ? `&status=${status}` : ''}${
      startDate ? `&start_date=${startDate}` : ''
    }${endDate ? `&end_date=${endDate}` : ''}`,
  );
};

export const getAppointmentsInfinite = async ({
  status,
  offset,
  limit,
  startDate,
