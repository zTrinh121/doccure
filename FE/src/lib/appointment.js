...
export const getAppointments = async ({ status, offset, limit, startDate, endDate }) => {
  return authAxiosInstance.get(`${appointmentPrefix}/all?offset=${offset}&limit=${limit}${status ? `&status=${status}` : ''}${startDate ? `&start_date=${startDate}` : ''}${endDate ? `&end_date=${endDate}` : ''}`)
}