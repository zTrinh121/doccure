...
import { doctorPrefix } from 'src/utils/apiConstants';
...
export const searchDoctors = async ({ input, spec }) => {
  ...
  if (input) {
    ...
    try {
      const response = await publicAxiosInstance.get(
        `${doctorPrefix}/specialization?specialization=${spec}`,
      );
...
export const getFilterDoctorByRating = async () => {
  return publicAxiosInstance.get(
    `${doctorPrefix}/rating/filter?type=avgRating&order=highToLow`,
  );
};
export const getDoctorSlots = async (startDate, endDate, doctorId) => {
  //yyyy-mm-dd
  return publicAxiosInstance.get(
    `${doctorPrefix}/slot/date?id=${doctorId}&start_date=${startDate}&end_date=${endDate}`,
  );
};
